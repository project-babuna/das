...
export async function sendPaymentReminder(lead) {
  if (!EMAIL_PATTERN.test(lead?.email || "")) return { skipped: true, reason: "missing_email" };

  const { data: currentLead, error } = await supabaseAdmin
    .from("leads")
  ...
  if (error) throw error;
  if (currentLead.payment_status === "success" || currentLead.status === "paid") {
    return { skipped: true, reason: "already_paid" };
  }

  const registeredAt = new Date(currentLead.created_at).getTime();
  if (!Number.isFinite(registeredAt) || Date.now() - registeredAt < REMINDER_DELAY_MS) {
  ...
  if (successfulPayment) {
    ...
    return { skipped: true, reason: "successful_payment_found" };
  }

  const program = getProgram(currentLead.interest);
  ...
  const body = `
    <p style="margin:0 0 14px;">Hi ${escapeHtml(firstName(currentLead.name))},</p>
  ...
  return sendLoggedEmail({
    ...
    html: emailShell({
      ...
      ctaHref: retryHref,
      footnote: "This reminder was sent because your registration remained unpaid for 12 hours.",
    }),
    text: `Hi ${firstName(currentLead.name)}, your ${program.title} registration is waiting for payment. Complete it here: ${retryHref}`,
    idempotencyKey: `payment-reminder-12h/${currentLead.id}`,
